...
public class TestDataProvider extends DefaultHandler
{
    ...
    public synchronized static Object[][] createTestData(final ITestContext context, Method method)
    {
        ...
        {
            Assert.fail("Unable to open file: " + context.getCurrentXmlTest().getParameter("testdataxml")
                    + "! Please check the file name.\n\n");
        } catch (SAXException realCause)
        {
            Assert.fail("SAX Exception!", realCause);
        } catch (ParserConfigurationException realCause)
        {
            Assert.fail("Parser Error!", realCause);
        }

        if (dataMap == null)
        {
            Assert.fail("TestData Not Found For Class: " + declaredClass + "!" + " Please check the xml data file.");
        }
